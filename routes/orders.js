...
router.post('/', (req, res) => {
  ...
  items.forEach(item => {
    db.run('INSERT INTO order_items (order_id, menu_item_id, name, price, quantity) VALUES (?, ?, ?, ?, ?)',
      [order.id, item.menu_item_id, item.name, item.price, item.quantity]);
  });
  db.save();

  const savedItems = db.all2('SELECT * FROM order_items WHERE order_id = ?', [order.id]);
